...
GLSphereSharp::GLSphereSharp(float center_x, float center_y, float center_z, float radius, int rows, int segments )
{
 ...
 */
void GLSphereSharp::initShader(void)
{
#ifdef _WIN32
    // This loads the shader program from a file
    _program = LoadAndCreateShaderProgram("../data/shaders/HW3_p2_sphere3.vs", "../data/shaders/HW3_p2_sphere3.fs");
#else
	 // This loads the shader program from a file
    _program = LoadAndCreateShaderProgram("../HW3_p2_sphere3.vs", "../HW3_p2_sphere3.fs");
#endif
    ...
    _material._specular_material = glm::vec3(0.0, 0.0, 0.0);
    _material._shininess = 12.0;
    
    _material1._diffuse_material = glm::vec3(0.0, 0.0, 0.0);
    _material1._ambient_material = glm::vec3(0.0, 0.0, 0.0);
    ...
    _material1._ambientColorPos = glGetUniformLocation(_program, "ambient_color1");
    _material1._diffuseColorPos = glGetUniformLocation(_program, "diffuse_color1");
    _material1._specularColorPos = glGetUniformLocation(_program, "specular_color1");
    _material1._shininessIdx = glGetUniformLocation(_program, "shininess1");
    ...
    _light_source1._lightPos = glm::vec4(50.0,80.0,0.0,1.0);
    _light_source1._ambient_intensity = 0.00;
    _light_source1._specular_intensity = 0.0;
    _light_source1._diffuse_intensity = 1000.0;
    _light_source1._attenuation_coeff = 0.00001;
    _light_source1._cone_angle = 90.0; // in degree
    _light_source1._cone_direction = glm::vec3(0, -1.0, 0.0); // this must be aligned with the object and light position.
    
    
    _light_source2._lightPos = glm::vec4(60.0,7.0,10.0,1.0);
    _light_source2._ambient_intensity = 0.3;
    _light_source2._specular_intensity = 10.0;
    _light_source2._diffuse_intensity = 0.1;
    _light_source2._attenuation_coeff = 0.02;
    _light_source2._cone_angle = 90.0; // in degree
    _light_source2._cone_direction = glm::vec3(-1.0, -1.0, -1.0); // this must be aligned with the object and light position.
